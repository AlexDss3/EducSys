...
class PlanificacionesControlador extends CI_Controller {
	...
	{
		if($this->input->post()){
			$this->PlanificacionesModel->Duplicar($this->input->post('id'));
			echo 'ok';
		}else{
			echo 'error';
		}
	...
	{
		if($this->input->post()){
			$data['ListaPlanificaciones'] 	= $this->PlanificacionesModel->Buscar($this->input->post('profesor'), $this->input->post('materia'), $this->input->post('seccion'));
	...
	public function BuscaAsignacion(){
		if ($this->input->post()) 
		{
			...
			$data = $this->PlanificacionesModel->BuscaAsignado($id, $an);

			$asignado = $data[0]->idAsignacion;
			$anio = $data[0]->anio;

			if ($asignado == $id && $anio == $an) {
				echo 'ok';
			}
		}
	}

	public function VerDetalles($idProfesor, $idPlanificacion, $idAsignacion, $anio, $materia)
	{
		$data['Unombre'] = $this->Unombre;
		$data['user'] = $this->user;
		$data['rol'] = $this->rol;
		...
		$data['NombreProfesor'] = $this->PlanificacionesModel->NombreProfesor($idProfesor);
